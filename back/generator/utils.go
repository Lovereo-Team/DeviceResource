...
// need change limit and offset be arg...
func (gu genUtil) GetDbTablesQuery(db *gorm.DB, tableName string, tableComment string, limit int, offset int) *gorm.DB {
	whereStr := ""
	var args []interface{}
	if tableName != "" {
	...
	queryStr := `table_schema = (SELECT database()) 
            AND table_name NOT LIKE "qrtz_%" 
            AND table_name NOT LIKE "gen_%" 
            AND table_name NOT IN (SELECT table_name FROM la_gen_table)` + whereStr + ` LIMIT ?` + ` OFFSET ?`

	args = append(args, limit, offset)
	query := db.Table("information_schema.tables").Where(queryStr, args...).Select(
		"table_name, table_comment, create_time, update_time")
	return query
}

func (gu genUtil) GetDbTablesQueryALL(db *gorm.DB, tableName string, tableComment string) *gorm.DB {
	whereStr := ""
	var args []interface{}
	if tableName != "" {
		whereStr += ` AND lower(table_name) like lower("%` + tableName + `%")`
	...
	queryStr := `table_schema = (SELECT database()) 
            AND table_name NOT LIKE "qrtz_%" 
            AND table_name NOT LIKE "gen_%" 
            AND table_name NOT IN (SELECT table_name FROM la_gen_table)` + whereStr
