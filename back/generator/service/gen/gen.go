...
func (genSrv generateService) DbTables(page request.PageReq, dbReq req.DbTablesReq) (res response.PageResp, e error) {
	...
	// 执行查询
	rows, err := tbModel.Rows()
	if err != nil {
		panic("failed to fetch table metadata: " + err.Error())
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		// 将 createTime 转换为 core.OnlyRespTsTime 类型
		var createTimeValue core.OnlyRespTsTime
		if createTime.Valid {
			createTimeValue = core.OnlyRespTsTime(createTime.Time)
		}

		// 将 updateTime 转换为 core.OnlyRespTsTime 类型
		var updateTimeValue core.OnlyRespTsTime
		if updateTime.Valid {
			updateTimeValue = core.OnlyRespTsTime(updateTime.Time)
		}

		// 构建 DbTableResp 对象并添加到结果列表
		tbResp = append(tbResp, resp.DbTableResp{
			TableName:    tableName,
